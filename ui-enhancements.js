...
function recoverClab(){if(bindClabToolbar())return;if(window.__mclClabRecoveryLoading)return;window.__mclClabRecoveryLoading=true;const s=document.createElement('script');s.src='clab-document-v1.js?v=1.1.18-recovery';s.onload=()=>{window.__mclClabRecoveryLoading=false;setTimeout(bindClabToolbar,20)};s.onerror=()=>{window.__mclClabRecoveryLoading=false;const n=document.getElementById('composerNote');if(n)n.textContent='CLAB-Modul konnte nicht geladen werden. Bitte die Seite neu laden.'};document.head.appendChild(s)}
