(() => {
  // Music Chat Lab v0.4.7 — composition path aligned to Composition Lab Engine Build 14.
  // Normal chat/analysis remains unchanged. Composition requests use the Build-14
  // system/concept/composition prompt flow and score representation.
  ...
  async function anthropicCall(url, headers, model, system, user, wantJson) {
    const body = {
      model,
      max_tokens: 32000,
    ...
    const r = await nativeFetch(url, { method: 'POST', headers, body: JSON.stringify(body) });
    if (!r.ok) return r;
    return r;
  }
  ...
  window.fetch = async function(input, init = {}) {
    ...
    try {
      ...
      const conceptResponse = await anthropicCall(url, init.headers, originalBody.model, SYSTEM_PREFIX, conceptPrompt, false);
      ...
      return anthropicCall(url, init.headers, originalBody.model, SYSTEM_PREFIX, compPrompt, true);
    } catch (_) {
      return nativeFetch(input, init);
    }
  };
  ...
  function buildMidi(score){
    ...
    (score.tr||[]).forEach((tr,ix)=>{
      ...
      (tr.ct||[]).forEach((c,i)=>ev.push({t:Math.round(Number(c[0])*PPQ),o:10+i,b:[0xb0|ch,Math.max(0,Math.min(127,Number(c[1])||0)),Math.max(0,Math.min(127,Number(c[2])||0))]}));
      (tr.nt||[]).forEach((n,i)=>{
  ...
  function enhance(){
    document.querySelectorAll('.message-row.assistant').forEach(row=>{
      ...
      const score=extractScore(bubble.textContent||''); if(!score){row.dataset.engine14Enhanced='1';return;}
      const name=filename(score); bubble.textContent=score.sm||'MIDI-Komposition wurde erzeugt.';
      const btn=document.createElement('button'); btn.className='generated-file-button'; btn.textContent=`⬇ ${name}`;
      btn.addEventListener('click',()=>download(buildMidi(score),name)); bubble.appendChild(document.createElement('br')); bubble.appendChild(btn);
