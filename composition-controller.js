(()=>{
'use strict';
if(window.__mclCompositionControllerV121)return;
window.__mclCompositionControllerV121=true;
...
function scoreInfo(s){let notes=0,end=0;for(const t of s?.score?.tr||[])for(const n of t?.nt||[])if(Array.isArray(n)){notes++;end=Math.max(end,(Number(n[0])||0)+(Number(n[1])||0))}const ts=s?.score?.ts||{},has=Number(ts.n)>0&&Number(ts.d)>0,bar=has?Number(ts.n)*(4/Number(ts.d)):null;return{slot:s?.slot??null,name:s?.name||'Quelle',notes,beats:Number(end.toFixed(3)),bars:bar?Math.max(1,Math.ceil((end-1e-7)/bar)):null,bpm:s?.score?.bpm??null,meter:has?`${ts.n}/${ts.d}`:null,key:s?.score?.k??null}}
function catalogue(sources){return sources.map((s,i)=>{const x=scoreInfo(s);return`${i+1}. ${x.slot?`Speicherplatz ${x.slot}: `:''}${x.name} | ${x.notes} Noten | ${x.beats} Beats${x.bars?` | ${x.bars} Takte`:''} | ${x.meter||'?'} | ${x.bpm??'?'} BPM | ${x.key||'Tonart frei'}`}).join('\n')||'Keine musikalische Quelle.'}
...
function synthetic(provider,text,model){if(provider==='anthropic')return new Response(JSON.stringify({id:'mcl-v121',type:'message',role:'assistant',model,content:[{type:'text',text}],stop_reason:'end_turn'}),{status:200,headers:{'content-type':'application/json'}});if(provider==='openai')return new Response(JSON.stringify({id:'mcl-v121',object:'response',model,output_text:text,output:[{type:'message',role:'assistant',content:[{type:'output_text',text}]}]}),{status:200,headers:{'content-type':'application/json'}});return new Response(JSON.stringify({candidates:[{content:{role:'model',parts:[{text}]},finishReason:'STOP'}]}),{status:200,headers:{'content-type':'application/json'}})}
...
function parseProposal(raw){
  const x=parseObject(raw);if(x&&typeof x.concept==='string'){const mode=String(x.mode||'').toUpperCase();return{concept:x.concept.trim(),mode:['PATCH','REPLACE_SCORE','NEW_SCORE'].includes(mode)?mode:null,base:x.base==null?null:Math.trunc(Number(x.base))}}
  let text=String(raw||'').trim().replace(/^```(?:text|markdown)?\s*/i,'').replace(/\s*```$/,'').trim();
  const mm=text.match(/(?:^|\n)\s*(?:MCL_)?MODE\s*[:=]\s*(PATCH|REPLACE_SCORE|NEW_SCORE)(?:\s*[;,]\s*BASE\s*[:=]\s*(\d+))?/i);
  const mode=mm?mm[1].toUpperCase():null,base=mm?.[2]?Number(mm[2]):null;
  text=text.replace(/(?:^|\n)\s*(?:MCL_)?MODE\s*[:=][^\n]*/i,'').replace(/^\s*(?:KONZEPT|CONCEPT)\s*:\s*/i,'').trim();
  return{concept:text,mode,base};
}
async function resolveMode(provider,url,headers,model,task,sources,concept){
  const prompt=`Bestimme nur den technischen Ergebnismodus für die spätere Ausführung. Komponiere nichts.\nPATCH = mindestens ein Teil einer ausgewählten vorhandenen Quelle bleibt exakt unverändert.\nREPLACE_SCORE = vorhandene Quelle wird als Ganzes durch eine neue Fassung ersetzt.\nNEW_SCORE = neue Musik ohne verwendete Quelle.\n\nAUFTRAG:\n${task}\n\nKOMPONITIONSIDEE:\n${concept}\n\nAUSGEWÄHLTE QUELLEN (nur Katalog, keine Notendaten):\n${catalogue(sources)}\n\nAntworte nur als JSON {"mode":"PATCH|REPLACE_SCORE|NEW_SCORE","base":1}.`;
  const raw=await rawCall(provider,url,headers,model,prompt,{maxTokens:120,timeout:90000}),x=parseObject(raw),mode=String(x?.mode||'').toUpperCase();if(!['PATCH','REPLACE_SCORE','NEW_SCORE'].includes(mode)){const e=new Error('Der technische Ergebnismodus konnte nicht zuverlässig bestimmt werden.');e.rawOutput=raw;throw e}let base=x.base==null?null:Math.trunc(Number(x.base));if(mode==='NEW_SCORE')base=null;else if(!(base>=1&&base<=sources.length))base=sources.length?1:null;return{mode,base,raw};
}
async function makeProposal(provider,url,headers,model,task,sources,change=null,oldConcept=''){
  const prompt=`KOMPONITIONSIDEE. Komponiere noch keine Noten.\nFormuliere eine kurze Kompositionsidee, die den freien Auftrag wirklich trifft. Beginne mit „Auftrag verstanden:“ und fasse zuerst knapp zusammen, was erzeugt oder verändert und was gegebenenfalls unverändert bleiben soll. Danach höchstens drei kurze Sätze zum musikalischen Gedanken. Keine detaillierte Bauanleitung.\nDie vollständigen Notendaten werden absichtlich erst bei der späteren Ausführung bereitgestellt. Nutze hier nur den Katalog, damit die Konzeptphase klein und robust bleibt.\nSetze vor den Konzepttext nach Möglichkeit genau eine technische Kopfzeile: MCL_MODE=PATCH;BASE=1 oder MCL_MODE=REPLACE_SCORE;BASE=1 oder MCL_MODE=NEW_SCORE.\n\nAUFTRAG:\n${task}\n${change?`\nÄNDERUNGSWUNSCH ZUR BISHERIGEN IDEE:\n${change}\n\nBISHERIGE IDEE:\n${oldConcept}`:''}\n\nAUSGEWÄHLTE QUELLEN (KATALOG OHNE NOTENDATEN):\n${catalogue(sources)}`;
  const raw=await rawCall(provider,url,headers,model,prompt,{maxTokens:700,timeout:120000});let q=parseProposal(raw);
  if(!q.concept){const e=new Error('Die Kompositionsidee war leer.');e.rawOutput=raw;throw e}
  if(!q.mode){const m=await resolveMode(provider,url,headers,model,task,sources,q.concept);q={...q,mode:m.mode,base:m.base}}
  if(q.mode==='NEW_SCORE')q.base=null;else if(!(q.base>=1&&q.base<=sources.length))q.base=sources.length?1:null;
  return{concept:q.concept,mode:q.mode,base:q.base,raw};
}
...
async function execute(provider,url,headers,model,rec){const p=rec.p,prompt=p.mode==='PATCH'?patchPrompt(p):fullPrompt(p),max=p.mode==='PATCH'?12000:32000;diag('v121-final-call',{provider,model,pendingId:rec.pid,task:p.task,concept:p.concept,mode:p.mode,base:p.base,sources:(p.sources||[]).map(scoreInfo)});let raw=await rawCall(provider,url,headers,model,prompt,{maxTokens:max,timeout:p.mode==='PATCH'?300000:360000}),score=materialize(raw,p);if(!score){diag('v121-final-retry',{provider,model,pendingId:rec.pid,mode:p.mode,outputCharacters:String(raw).length,rawOutput:String(raw).slice(0,5000)});raw=await rawCall(provider,url,headers,model,prompt+`\nKORREKTUR: Die vorige Antwort war für den verbindlichen Modus ${p.mode} ungültig. Gib ausschließlich das verlangte JSON aus.`,{maxTokens:max,timeout:p.mode==='PATCH'?300000:360000});score=materialize(raw,p)}if(!score){const e=new Error(`Die KI hat kein gültiges Ergebnis im bestätigten Modus ${p.mode} geliefert.`);e.rawOutput=raw;throw e}storeScoreMeta(score,p,provider,model);delete rec.store[rec.pid];save(PENDING,rec.store);diag('v121-final-valid',{provider,model,pendingId:rec.pid,task:p.task,concept:p.concept,mode:p.mode,base:p.base,tracks:score.tr.length,outputCharacters:String(raw).length,scoreFingerprint:fingerprint(score)});return synthetic(provider,JSON.stringify(score),model)}
...
window.fetch=async function(input,init={}){
  ...
  try{
    ...
    if(rec&&rejectText(task)){delete rec.store[rec.pid];save(PENDING,rec.store);diag('v121-proposal-rejected',{provider,model,pendingId:rec.pid});return synthetic(provider,'Kompositionsidee verworfen. Es wurde keine Komposition erzeugt.',model)}
    const candidates=mergeSources(embeddedSources(user.content),workspaceSources()),r=await route(provider,url,init.headers,model,msgs,candidates,!!rec);diag('v121-routed',{provider,model,intent:r.intent,userText:task,active:scoreInfo(activeWorkspace()||{}),sources:r.sources.map(scoreInfo),hasPending:!!rec});
    ...
    if(rec&&r.intent==='REVISE'){const q=await makeProposal(provider,url,init.headers,model,rec.p.task,rec.p.sources,task,rec.p.concept);rec.p={...rec.p,...q,createdAt:Date.now()};rec.store[rec.pid]=rec.p;save(PENDING,rec.store);diag('v121-proposal-revised',{provider,model,pendingId:rec.pid,task:rec.p.task,concept:q.concept,mode:q.mode,base:q.base,rawProposal:String(q.raw||'').slice(0,4000)});return synthetic(provider,visibleProposal(rec.pid,q.concept),model)}
    ...
    const q=await makeProposal(provider,url,init.headers,model,task,r.sources),pid=id(),store=load(PENDING);store[pid]={task,concept:q.concept,mode:q.mode,base:q.base,sources:r.sources.map(clone),provider,model,createdAt:Date.now()};save(PENDING,store);diag('v121-proposal-created',{provider,model,pendingId:pid,task,concept:q.concept,mode:q.mode,base:q.base,sources:r.sources.map(scoreInfo),rawProposal:String(q.raw||'').slice(0,4000)});return synthetic(provider,visibleProposal(pid,q.concept),model);
  }catch(e){const msg=e?.message||String(e);diag('v121-error',{provider,model,userText:task,error:msg,rawOutput:e?.rawOutput?String(e.rawOutput).slice(0,5000):undefined});return errorResponse(msg)}
};
