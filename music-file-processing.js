(function () {
  ...
  function parseMidi(arrayBuffer) {
    ...
    for (let ti = 0; ti < tracks && pos + 8 <= data.length; ti++) {
      ...
      const end = Math.min(data.length, pos + 8 + len);
      ...
      while (state.pos < end) {
        ...
        if (status === 0xff) {
          ...
          state.pos = Math.min(end, state.pos + mlen);
          if (type === 0x03) name = new TextDecoder().decode(data.slice(start, start + mlen));
            ...
            keySigs.push({ tick, fifths, minor: data[start + 1] === 1 });
          }
          totalEvents++;
          continue;
        }
        if (status === 0xf0 || status === 0xf7) {
          const slen = readVar(data, state); state.pos = Math.min(end, state.pos + slen); totalEvents++; continue;
        }
    ...
    return { format, tracks, division, ppq, maxTick, tempos, timeSigs, keySigs, trackData: outTracks, totalEvents, truncated: false };
  }
  ...
  function midiToText(parsed, fileName) {
    ...
    if (parsed.keySigs?.length) lines.push(`Tonart-Metaevents: ${parsed.keySigs.slice(0, 12).map(k => `${k.fifths} Quinten, ${k.minor ? 'Moll' : 'Dur'} @${k.tick}`).join(', ')}`);
    parsed.trackData.forEach(t => {
      ...
      if (t.controllers.length) lines.push(`Controller: ${t.controllers.slice(0, 120).map(c => `Ch${c.channel} CC${c.controller}=${c.value}@${c.tick}`).join(', ')}`);
