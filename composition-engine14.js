(() => {
  ...
  // Since v1.1.5 this module only provides score/MIDI conversion and UI output.
  // Conversation/composition intent is handled exclusively by concept-approval.js
  // and concept-openai.js. There is deliberately NO API/fetch interception here.
      ...
      for(const channel1 of channels){
        ...
        tracks.push({
          ...
          nt:notes.map(n=>[
            ...
            Number(n.note),Number(n.velocity),0,1.0
          ]),
  ...
  function mtrk(events){
    ...
    return[...bytes('MTrk'),...u32(out.length),...out];
  }
  ...
  function buildMidi(score){
    ...
    (score.tr||[]).forEach((tr,ix)=>{
      ...
      const ev=[],nm=bytes(tr.nm||`Track ${ix+1}`);
