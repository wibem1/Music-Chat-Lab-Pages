(() => {
  // v0.4.21 — Claude + Gemini: robust proposal state; only the immediate reply can act on a proposal.
  ...
  function diagnostic(stage,data){try{localStorage.setItem(DIAG,JSON.stringify({version:'0.4.21',timestamp:new Date().toISOString(),stage,...data},null,2))}catch{}}
    ...
    try{
      ...
      if(pendingId&&pending[pendingId]){
        ...
        if(yesRe.test(change)){
          ...
          const text=await direct(provider,url,init.headers,model,compPrompt,true);return providerResponse(provider,text,model);
        }
